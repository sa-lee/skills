---
name: tikz
description: Audit and fix residual TikZ visual collisions in any .tex file. A downstream repair tool — not a safety net. The upstream defense is /beautiful-deck-quarto Step 4.4, which writes safe TikZ from the start. Use /tikz when labels overlap arrows, text sits on boxes, or arrows cross each other. Applies mathematical gap calculations and Bézier depth formulas — no eyeballing.
...
**The upstream defense is `/beautiful-deck-quarto` Step 4.4**, which writes safe TikZ from the start:
...
Full rules and formulas are in `~/.claude/skills/beautiful-deck-quarto/tikz_rules.md`. Read it if you haven't. What follows is the operational checklist — the reference file has the formulas.
