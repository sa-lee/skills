...
> (`presentations/rhetoric_of_decks.md`). It is the operational
> philosophy that the `beautiful-deck-quarto` skill enforces.
>
> `beautiful-deck-quarto` is adapted from Scott's `/beautiful_deck`
> skill (`~/repos/MixtapeTools/skills/beautiful_deck/`), with WEHI
> brand pack, Quarto-Beamer specifics, and statistical-genetics
> audience defaults added by Stuart Lee. The Mixtape repository was
> open to vendoring at the time of copying (no LICENSE file present;
> permission per the author).
